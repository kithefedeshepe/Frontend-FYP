...
function DoctorViewResult() {
  ...
  const fetchReportDetails = async (reportId, token) => {
    try {
      const response = await axios.get(`https://43.134.34.32.nip.io/api/doctor/getReport/${reportId}/`, {
        headers: {
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      const reportId = rid || localStorage.getItem('selectedReportId');
      const response = await axios.put(`https://43.134.34.32.nip.io/api/doctor/updateReport/${reportId}/`, {
